...
def get_data(conditions, filters):
	time_sheet = frappe.db.sql("""
		select 
		    ...
		    `tabTimesheet Detail`.note,
		    `tabTimesheet`.status 
		from `tabTimesheet Detail`
                inner join `tabTimesheet` ON `tabTimesheet Detail`.parent = `tabTimesheet`.name
                inner join `tabProject` ON `tabTimesheet Detail`.project = `tabProject`.name
		where %s 
		order by `tabTimesheet`.name"""%(conditions), filters, as_list=1)
...
def get_conditions(filters):
	...
	if filters.get("customer"):
		conditions += " and `tabProject`.customer = %(customer)s"
